...
class GroqProvider(BaseAIProvider):
    def __init__(self):
        ...
        self.client = None
        if self.api_key:
            # Groq is OpenAI-compatible and can use standard AsyncOpenAI client
...
"""
        try:
            logger.info("Calling Groq API to generate questions...")
            ...
            logger.error(f"Groq generate_questions failed: {e}")
            raise e
...
"""
        try:
            logger.info("Calling Groq API to evaluate interview...")
            response = await client.chat.completions.create(
                model="llama-3.1-8b-instant",
                messages=[
                    {
                        ...
                        "content": (
                            ...
                            "Keep feedback brief and token-efficient."
                        )
                    },
                    {"role": "user", "content": prompt}
                ],
            ...
            logger.error(f"Groq evaluate_interview failed: {e}")
            raise e
